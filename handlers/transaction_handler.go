...
func (h *TransactionHandler) HandleReportToday(w http.ResponseWriter, r *http.Request) {
	...
	report, err := h.service.GenerateTodayReport()
	if err != nil {
...
func (h *TransactionHandler) HandleReport(w http.ResponseWriter, r *http.Request) {
	...
	if startDate == "" || endDate == "" {
		...
		endDate = fmt.Sprintf("%d-%02d-%02d", now.Year(), now.Month(), daysInMonth(now.Year(), now.Month()))
	}

	report, err := h.service.GenerateReport(startDate, endDate)
	if err != nil {
		http.Error(w, "Failed to generate report", http.StatusInternalServerError)
		return
	}
